...
function analyzeCorpus() {
    ...
    setTimeout(async function() {
        ...
        try {
            ...
            if (aiResult) {
                html += '<div class="interpretation-box"><h3><i class="fas fa-lightbulb"></i> Interprétation Linguistique</h3><p>' + aiResult.replace(/\n/g, '<br>') + '</p></div>';
                resultsDiv.innerHTML = html;
            }
        } catch (e) {}
...
function generateCorpusHTML(analysis) {
    ...
    html += '<h3 style="margin-top:30px;"><i class="fas fa-trophy"></i> Mots Fréquents</h3>';
...
function exportToPDF(toolType) {
    ...
    doc.save('LinguaLab_' + toolType + '.pdf');
}
